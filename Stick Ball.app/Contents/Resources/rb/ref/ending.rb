...
class Ending < Chingu::GameState
  ...
  def setup
    ...
    @text = Chingu::Text.create("The Earth has been saved.", :y => 900, :font => Gosu.default_font_name, :size => 60, :color => Colors::White, :zorder => 300)
...
class EndCredits < Chingu::GameState
  ...
  def setup
    ...
    @t1 = Chingu::Text.create(:text=>"" , :y=>675, :size=>40, :font => Gosu.default_font_name)
    @t1.x = 400 - @t1.width/2   # @t1 is currently blank
    @t2 = Chingu::Text.create(:text=>"Produced by Fractional and mpl" , :y=>@st+@sp*2, :size=>40, :font => Gosu.default_font_name)
    @t2.x = 400 - @t2.width/2
    @t3 = Chingu::Text.create(:text=>"Gosu by jlnr" , :y=>@st+@sp*3, :size=>40, :font => Gosu.default_font_name)
    @t3.x = 400 - @t3.width/2
    @t4 = Chingu::Text.create(:text=>"Chingu by ippa" , :y=>@st+@sp*4, :size=>40, :font => Gosu.default_font_name)
    @t4.x = 400 - @t4.width/2
    @t5 = Chingu::Text.create(:text=>"Ruby by Yukihiro Matsumoto" , :y=>@st+@sp*5, :size=>40, :font => Gosu.default_font_name)
    @t5.x = 400 - @t5.width/2
    @t6 = Chingu::Text.create(:text=>"Game Theme Music by ExplodingCookie " , :y=>@st+@sp*6, :size=>40, :font => Gosu.default_font_name)
    @t6.x = 400 - @t6.width/2
    @t7 = Chingu::Text.create(:text=>"Intro Music from StarryKnight by burtlo" , :y=>@st+@sp*7, :size=>40, :font => Gosu.default_font_name)
    @t7.x = 400 - @t7.width/2
    @t8 = Chingu::Text.create(:text=>"Knight and Voice from StarryKnight by burtlo" , :y=>@st+@sp*8, :size=>40, :font => Gosu.default_font_name)
    @t8.x = 400 - @t8.width/2

    @t9 = Chingu::Text.create(:text=>"End Music needs attribution" , :y=>@st+@sp*9, :size=>40, :font => Gosu.default_font_name)
    @t9.x = 400 - @t9.width/2
    @t10 = Chingu::Text.create(:text=>"Sound Effects need additional attribution" , :y=>@st+@sp*10, :size=>40, :font => Gosu.default_font_name)
    @t10.x = 400 - @t10.width/2
    @t11 = Chingu::Text.create(:text=>"Future Earth by" , :y=>@st+@sp*11, :size=>40, :font => Gosu.default_font_name)
    @t11.x = 400 - @t11.width/2
    @img11 = Signature1.create
    @img11.x = 400 + @t11.width/2 + 5 + @img11.width/2
    @img11.y = @st+@sp*11+20
    @t12 = Chingu::Text.create(:text=>"Future Earth 2 by" , :y=>@st+@sp*12, :size=>40, :font => Gosu.default_font_name)
    @t12.x = 400 - @t12.width/2
    @img12 = Signature2.create
    @img12.x = 400 + @t12.width/2 + 10 + @img12.width/2
    @img12.y = @st+@sp*12+20
    @t13 = Chingu::Text.create(:text=>"Additional thanks to Linux/Ubuntu" , :y=>@st+@sp*13, :size=>40, :font => Gosu.default_font_name)
    @t13.x = 400 - @t13.width/2
    @t14 = Chingu::Text.create(:text=>"Additional thanks to rvm" , :y=>@st+@sp*14, :size=>40, :font => Gosu.default_font_name)
    @t14.x = 400 - @t14.width/2
    @t15 = Chingu::Text.create(:text=>"Additional thanks to SublimeText2" , :y=>@st+@sp*15, :size=>40, :font => Gosu.default_font_name)
    @t15.x = 400 - @t15.width/2
    @t16 = Chingu::Text.create(:text=>"Additional thanks to Gimp" , :y=>@st+@sp*16, :size=>40, :font => Gosu.default_font_name)
    @t16.x = 400 - @t16.width/2
    @t17 = Chingu::Text.create(:text=>"Additional thanks to gamedev.stackexchange.com" , :y=>@st+@sp*17, :size=>40, :font => Gosu.default_font_name)
    @t17.x = 400 - @t17.width/2
    @t18 = Chingu::Text.create(:text=>"Special thanks to PeterT and Spooner" , :y=>@st+@sp*19, :size=>40, :font => Gosu.default_font_name)
    @t18.x = 400 - @t18.width/2
    @t19 = Chingu::Text.create(:text=>"Extra Special thanks to jlnr and libgosu.org" , :y=>@st+@sp*20, :size=>40, :font => Gosu.default_font_name)
    @t19.x = 400 - @t19.width/2
    @t20 = Chingu::Text.create(:text=>"Thanks to the people of Sweden and Honduras" , :y=>@st+@sp*26, :size=>50, :font => Gosu.default_font_name)
    @t20.x = 400 - @t20.width/2
    @t21 = Chingu::Text.create(:text=>"Thanks to the people of Japan and Germany" , :y=>@st+@sp*27, :size=>50, :font => Gosu.default_font_name)
